...
import { generateWrapped } from "@/lib/claude";
...
export async function POST(req: NextRequest) {
  try {
    const { username, platform, year } = await req.json();
    if (!username?.trim()) return NextResponse.json({ error: "Username required" }, { status: 400 });

    const targetYear = year ?? new Date().getFullYear();

    const ip = req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() ?? "unknown";
    const count = await cacheIncr(`ratelimit:wrapped:${ip}`, 86400);
    if (count > 5) return NextResponse.json({ error: "Daily limit reached." }, { status: 429 });

    const cacheKey = `wrapped:${platform ?? "mal"}:${username.toLowerCase()}:${targetYear}`;
    ...
    const result = await generateWrapped(targetYear, summary);
    await cacheSet(cacheKey, result, 3600 * 24 * 7);
    return NextResponse.json({ ...result, username: summary.username, year: targetYear });
  } catch (err: unknown) {
